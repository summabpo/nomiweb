...
(function (global) {
  ...
  function recalculateSettlementTotals(modalContent) {
    ...
    var diasCesEfectivos = Math.trunc(getNumber(modalContent, 'dias_cesantias'));
    var diasSuspCes = Math.trunc(getNumber(modalContent, 'dias_susp_ces'));
    var diasInteresesCes = diasCesEfectivos + diasSuspCes;
    ...
    var cesantias = Math.ceil((baseCes * diasCesEfectivos) / 360);
    var prima = Math.ceil((diasPri / 360) * basePri);
    var vacaciones = Math.ceil((baseVac * diasVac) / 30);
    var intereses = Math.ceil(cesantias * 0.12 * (diasInteresesCes / 360));
