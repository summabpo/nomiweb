...
from apps.common.models import NovFijos , Conceptosdenomina , Contratos,Nomina
...
def fixed_modal_edit(request,id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            response['X-Up-Accept-Layer'] = 'true'  #Indica a Unpoly que acepte (cierre) el modal
            response['X-Up-icon'] = 'success'  # URL para recargar la página principal   
            response['X-Up-message'] = 'La Novedad fue Actualizada correctamente'    
            response['X-Up-Location'] = reverse('payroll:fixedconcepts')           
            return response
...
@login_required
@role_required('accountant')
def fixed_modal_edit_des(request,id):
    """
    Muestra y procesa el formulario modal para registrar una novedad fija.
    ...
    form = FixidForm(idempresa=idempresa ,edit = False ,edit2 = True , initial = data)
    
    if request.method == 'POST':
        form = FixidForm(request.POST , idempresa=idempresa ,edit = False ,edit2 = True )
        if form.is_valid():
...
@login_required
@role_required('accountant')
def fixed_modal_views_historico(request,id):
    aux = False
    usuario = request.session.get('usuario', {})
    idempresa = usuario['idempresa']
    novfija = NovFijos.objects.get(estado_novfija=True ,idnovfija =  id  )
    items = Nomina.objects.filter(idconcepto = novfija.idconcepto ,control = novfija.idnovfija ).order_by('-idregistronom')

    if novfija.idconcepto.codigo == 91 : 
        aux = True
    ...
    return render(request, './payroll/partials/fixedconceptsmodal_historico.html' ,contex)
