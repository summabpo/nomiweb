...
class ContractForm(forms.Form):
    ...
    position = forms.ChoiceField(choices=[('', '----------')] + [(cargo.nombrecargo, cargo.nombrecargo) for cargo in Cargos.objects.all().order_by('nombrecargo')], label='Cargo', required=True)
    ...
    # Seguridad Social 
    eps = forms.ChoiceField(choices=[('', '----------')] + [(entidad.codigo, entidad.entidad) for entidad in Entidadessegsocial.objects.using("lectaen").filter(tipoentidad='EPS').order_by('entidad')], label='Eps', required=True) 
    pensionFund = forms.ChoiceField(choices=[('', '----------')] + [(entidad.codigo, entidad.entidad) for entidad in Entidadessegsocial.objects.using("lectaen").filter(tipoentidad='AFP').order_by('entidad')], label='Pension', required=True) 
    CesanFund = forms.ChoiceField(choices=[('', '----------')] + [(entidad.codigo, entidad.entidad) for entidad in Entidadessegsocial.objects.using("lectaen").filter(tipoentidad='AFP').order_by('entidad')], label='Fondo Cesantias', required=True) 
    workPlace = forms.ChoiceField(choices=[('', '----------')] + [(sede.idsede, sede.nombresede) for sede in Sedes.objects.using("lectaen").all()], label='Centro de Trabajo ARL', required=True)
    arlWorkCenter  = forms.ChoiceField(choices=[('', '----------')] + [(centro.centrotrabajo, centro.nombrecentrotrabajo) for centro in Centrotrabajo.objects.using("lectaen").all()], label='Sede de Trabajo', required=True)
