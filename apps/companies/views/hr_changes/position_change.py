...
@login_required
def get_contract_position(request):
    contract_id = request.GET.get('contract_id')

    try:
        contrato = Contratos.objects.select_related('cargo').get(idcontrato=contract_id)
        return JsonResponse({
            'cargo_id': contrato.cargo.idcargo if contrato.cargo else '',
            'cargo_nombre': contrato.cargo.nombrecargo if contrato.cargo else ''
        })
    except Contratos.DoesNotExist:
